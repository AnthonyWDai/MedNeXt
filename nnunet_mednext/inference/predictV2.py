...
def predict_cases(
    ...
    """
    MedNext version rewritten in the style of the second code version.
    """
...
def predict_cases_fast(
    ...
    for preprocessed in preprocessing:
        ...
        assert region_class_order is None, \
            "predict_cases_fast can only work with regular softmax predictions and is therefore " \
            "unable to handle trainer classes with region_class_order"
...
def predict_cases_fastest(
    ...
    for preprocessed in preprocessing:
        ...
        assert region_class_order is None, \
            "predict_cases_fastest can only work with regular softmax predictions and is therefore " \
            "unable to handle trainer classes with region_class_order"
...
def check_input_folder_and_return_caseIDs(input_folder, expected_num_modalities):
    ...
    if len(remaining) > 0:
        print(
            "found %d unexpected remaining files in the folder. Here are some examples:" % len(remaining),
...
def predict_from_folder(
    ...
    if lowres_segmentations is not None:
        assert isdir(lowres_segmentations), \
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input_folder", required=True,
                        help="Must contain all modalities for each patient in the correct order (same as training). "
                             "Files must be named CASENAME_XXXX.nii.gz where XXXX is the modality identifier.")
    parser.add_argument("-o", "--output_folder", required=True, help="folder for saving predictions")
    parser.add_argument("-m", "--model_output_folder", required=True,
                        help="model output folder. Will automatically discover the folds that were run and use those as an ensemble")
    parser.add_argument("-f", "--folds", nargs="+", default="None",
                        help="folds to use for prediction. Default is None which means folds will be detected automatically")
    parser.add_argument("-z", "--save_npz", required=False, action="store_true",
                        help="save softmax probabilities as compressed numpy arrays")
    parser.add_argument("-l", "--lowres_segmentations", required=False, default="None",
                        help="required for highres stage of cascade")
